    ...
    const haneldClick = (item: 'new' | 'simbrief' | 'server') => {
        ...
        switch (item) {
            ...
            case 'simbrief':
                ...
                if (!user){
                    return message.error('No company account found')
                }
                ...
                }).catch(() => message.error('Error when fetching simbrief info'))
                break
            case 'server':
                const username = getUserData()?.Username
                if (!username) return messageApi.error('Failed to load！')
    ...
    const handleDeleteFlight = (d: FieldType, e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
        ...
        axios.post(apiUrl.deleteFlights,{'flight': d},{'headers': createHeader()}).then(async res => {
            if (res.data.code === 200){
                ...
                setList(l)
                return message.success('Delete flight success')
            }
            return message.error('Delete flight failed')
        }).catch(() => message.error('Delete flight failed'))
    }
            ...
            bottom-0 rounded-r-md bg-[#2f4565] ani-show-common-panel phone:w-full phone:left-0 duration-300 ${editMode ? 'edit-mode' : 'placeholder-overview'}`}
                ...
                <PhoneDrag id={WINDOW} />
                  ...
                  select-none phone:mt-[-30px] mb-[12px]">
                    {detail ? data?.OFP.general.flightNumber : 'Flight List'}
                  </div>
                  {
                    (detail && !fail) ? 
                    <Popconfirm
                        placement="bottomRight"
                        title='Conform unload flight？'
                        okText="Yes"
                        cancelText="No"
                    ...
                    >
                        <div className="absolute top-[4px] right-[10px] mt-[4px] 
                            ...
                            cursor-pointer duration-200 phone:mt-[5px] pt-[2px] pb-[2px] pl-[6px] pr-[6px]">Unload
                        </div>
                    </Popconfirm>
                    :
                        <>
                            <div onClick={handleClose} className="absolute top-[4px] right-[10px] mt-[4px]  h-[20px]
                                ...
                                cursor-pointer phone:mt-[8px] pl-[6px] pr-[6px]">&lt;&nbsp;Back
                            </div>
                  ...
                  
                    {
                        ...
                        <div className="relative flex justify-around w-full h-[35px] mt-2 mb-2 flight-action">
                            ...
                            onClick={() => haneldClick('new')}>Create new flight</Button></div>
                            {/* <div><Button type="primary"
                            onClick={() => haneldClick('server')}>import from server</Button></div> */}
                            <div><Button type="primary"
                            onClick={() => haneldClick('simbrief')}>import from company</Button></div>
                        </div>
