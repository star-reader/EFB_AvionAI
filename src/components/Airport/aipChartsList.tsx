...
export default memo(() => {
    ...
    const haneleClickChart = (data: AIPChartList) => {
        ...
        axios.post(apiUrl.aipUrl,data, {'headers': createHeader()}).then(res => {
            ...
            message.error('Chart load failed')
        })
        // 设置点击航图样式
        ...
        <div className="relative overflow-y-auto w-full search-calc-height">
            {
                ...
                fail || !aipList.length ? <Result
                    status="warning"
                    title="Loading failed. No AIP Charts available in this airport~"
                    subTitle="Only support China Mainland currently. Other region will be added soon."
                /> :
